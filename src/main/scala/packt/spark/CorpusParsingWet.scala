...
import packt.{HelperScala, WarcWetRecord}
 ...
 */
object CorpusParsingWet {

  def main(args: Array[String]) = {

    val session: SparkSession = HelperScala.createSession(2, "Corpus Parsing Wet")
    ...
    val webpagesRDD: RDD[WarcWetRecord] = session.sparkContext
      .newAPIHadoopFile(inputLocationWet, classOf[TextInputFormat], classOf[LongWritable], classOf[Text], hadoopConf)
      .flatMap(parseRawWetRecord(_))
      .filter(_.warcType != "warcinfo") // skip meta header info for file

    val webpagesDataset: Dataset[WarcWetRecord] = webpagesRDD.toDS()
